...
**Amended:** 2026-06-16 by the Feature 4 brainstorm — the Weather Variables are carried in `CurrentConditions` as **UnitsNet quantity types** (`Temperature`/`Speed`/`Length`), the canonical representation Feature 4's formatter converts from; until F4 lands, the Current Conditions VM formats them in **interim fixed units**. See §7 **R6** and `docs/superpowers/specs/0004-per-measurement-unit-preferences.md` §7 R1.
...
**Amended (F4, §7 R6):** the seven Weather Variables are carried in `CurrentConditions` as **UnitsNet quantity types** — temperature/feels-like as `Temperature`, wind speed as `Speed`, precipitation as `Length` (humidity stays `double %`, wind direction stays degrees + 8-point compass — neither is user-choosable). The mapper (Seam 1) **wraps** the Open-Meteo numbers into quantities, so the units this section requests become an **internal fetch detail**; the **display** unit is Feature 4's formatter's job. Until F4 lands, the Current Conditions VM formats these quantities in the **fixed units** above — F4 swaps the fixed unit for the User's *effective* unit and adds the lossless, no-re-fetch re-render. The "no conversion/formatting layer in F2" point above is unchanged: F2 still ships **no** formatter; carrying a quantity is representation, not formatting.
...
- **R6 — Amended to carry UnitsNet quantities (Feature 4 brainstorm, 2026-06-16).** Feature 4 (Per-measurement Unit Preferences) needs a **canonical representation** its formatter can convert from **losslessly without re-fetching** (ADR 0002). Because F2 is **spec-only (not yet coded)** and implements **before** F4, the Feature owner chose to amend this spec **now** so `CurrentConditions` is built **quantity-native** rather than having F4 rework `double` fields. Delta: temperature/feels-like → `Temperature`, wind speed → `Speed`, precipitation → `Length` (humidity stays `double %`, wind direction stays degrees + compass — neither is user-choosable); the Seam 1 mapper wraps the Open-Meteo numbers into these quantities (§4.3). **No F2 scope change** — F2 still ships **no** formatter and still displays each variable in the **fixed units** of §4.3 (now as interim formatting of a quantity); the per-measurement choice, CLDR defaults, formatter, and the lossless no-re-fetch re-render are all **Feature 4**. This refines R2 (the "no formatter in F2" point stands; only the value *representation* changed). Cross-referenced from `docs/superpowers/specs/0004-per-measurement-unit-preferences.md` §7 R1.
