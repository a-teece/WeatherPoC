...
DailyForecastDay { DateOnly Date; double? HighC; double? LowC; WeatherCondition Condition; }
```

- `HighC`/`LowC` are nullable to honour Open-Meteo's nullable daily elements (§4.4); a null renders as "—", never a crash.
- `Condition` is the **same `(category, label, iconKey)` Weather Condition value** F2's WMO mapping yields — reused, not redefined (§4.5).
- High/low stay in **fixed °C** (matching F2 §4.3); changing the unit is a **Feature 4** concern and never a re-fetch.
...
The **Daily Forecast ViewModel** additionally computes each day's **label** (§4.6.1) and exposes the per-day `HighC`/`LowC`/`Condition` for binding.
...
- **(e) authority:** the **live Open-Meteo Forecast API** (`api.open-meteo.com/v1/forecast`; docs `open-meteo.com/en/docs`) — the `daily` variable names, `forecast_days`, `timezone=auto` day-bucketing semantics, the parallel-array response shape, and the WMO `weather_code` table. **Auth method:** inherited from F2's first contact with Open-Meteo — the service is **keyless** (no API key, no wire auth; Overriding Principle 1); F3 introduces no new auth and re-confirms none is required. **Grounding note:** live grounding could **not** be performed in this brainstorm's remote session — the egress allowlist blocks `api.open-meteo.com` and `open-meteo.com/en/docs` returns 403 to automated fetches. The contract therefore rests on F2's prior live grounding of the same endpoint plus the **binding proof being the end-to-end test**, which runs where egress is open at implementation time; the offline fixtures must be **captured from that real response**, never hand-invented (re-encoding an unchecked assumption is exactly what (d) forbids).
